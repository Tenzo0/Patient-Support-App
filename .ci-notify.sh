...
curl -s —max-time $TIME -d "chat_id=$TELEGRAM_USER_ID_2&disable_web_page_preview=1&text=$TEXT" $URL > /dev/null 
curl -s —max-time $TIME -d "chat_id=$TELEGRAM_USER_ID_3&disable_web_page_preview=1&text=$TEXT" $URL > /dev/null